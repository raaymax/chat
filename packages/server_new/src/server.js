...
const connections = require('./infra/connections');

const app = require('./infra/http');
const wss = require('./infra/ws');
...
wss({ server })
  ...
  .on('start', () => console.log('[WSS] Server is listening'))
...
async function auth(self, msg) {
  const self2 = connections.getByConnection(self.ws, msg.token);
  console.log(self2);
  msg.ok({self2});
} 
