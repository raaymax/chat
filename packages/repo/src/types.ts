import { z } from 'zod';
import {
  ObjectId, Document, Filter, InsertOneResult, WithId,
} from 'mongodb';

export const Id = z.custom<string & { __appId: true }>((val) => /^[a-f0-9]{24,}$/.test(val as string));
export const ClientId = z.custom<string & { __clientId: true }>((val) => /.+/.test(val as string));
...
export type ReplaceType<T, F, B> = {
  [P in keyof T]: T[P] extends F
    ? B
