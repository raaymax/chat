...
type MessageQuery = Partial<Message & {
  search: string,
  before: Date,
  after: Date,
}>;
...
class MessageRepo extends Repo<MessageQuery, Message>{
  ...
  async updateThread(arg: { parentId: EntityId, userId: EntityId, id: EntityId}) {
    const { db } = await connect();
    const data = serialize(arg);
    return db
      .collection<{thread: {userId: ObjectId, childId: ObjectId}[]}>(this.COLLECTION)
      .updateOne({
        _id: data.parentId,
      }, {
        $push: {
