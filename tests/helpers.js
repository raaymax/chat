...
const request = (ws) => (msg) => {
  ...
  return new Promise(resolve => {
    ...
    ws.addEventListener('message', (raw) => {
      ...
      if(msg.seqId !== id) return;
      list.push(msg);
      if(msg.resp) return resolve(list);
    })
...
const parts = [
  ...
  {match: (e) => true, run: e=> eq(e)},
]

const check = (e) => {
  return parts.find(i => i.match(e)).run(e)
}
...
function full(expected) {
  ...
  return function m(matched) {
    if(typeof matched === 'undefined') 
      return [{msg: 'is undefined expected: ' + JSON.stringify(expected)}];
...
const exactArray = expected => matched => {
  ...
  if(expected.length !== matched.length) return [{
    expected: expected.length,
    matched,
    msg: 'length not match'
  }];
  const res = expected.map((m, idx) => {
      return check(m)(matched[idx]);
  }).flat();
  return res;
}
