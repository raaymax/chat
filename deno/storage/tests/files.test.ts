...
for (const config of configs) {
  ...
  Deno.test(`[${TYPE}] text file upload and download`, async (t) => {
    ...
    await t.step('GET /:id - get file', async () => {
      ...
      assertEquals(res.headers.get('content-length'), file.byteLength.toString());
      assertEquals(res.headers.get('content-disposition'), `attachment; filename="test.txt"`);
    });
  ...
  Deno.test(`[${TYPE}] image upload and download`, async (t) => {
    ...
    await t.step('GET /:id - get file', async () => {
      ...
      assertEquals(res.headers.get('content-length'), file.byteLength.toString());
      assertEquals(res.headers.get('content-disposition'), `attachment; filename="quack.png"`);
    });
    ...
    await t.step('GET /:id - get image scaled', async () => {
      ...
      const file = await Deno.readFile(testImagePath);
      const body = await res.arrayBuffer();
      assert(body.byteLength < file.byteLength, 'scaled image should be smaller');
      assertEquals(res.headers.get('content-type'), 'image/png');
      assertEquals(res.headers.get('content-length'), body.byteLength.toString());
      assertEquals(res.headers.get('content-disposition'), `attachment; filename="quack.png"`);
    });
