...
export const files = (config: Config['storage']) => {
  ...
  return {
    upload: async (stream: ReadableStream<Uint8Array>, options: FileOpts): Promise<string> => {
      ...
      const file = await Deno.open(path.join(dir, id), { write: true, create: true, append: false, truncate: true});
      const s = stream.tee();
      ...
      await Deno.writeTextFile(path.join(dir, `${id}.json`), JSON.stringify({...options, size}));
      return id;
    },
    get: async (id: string): Promise<FileData> => {
      ...
      const meta = JSON.parse(await Deno.readTextFile(path.join(dir, `${id}.json`)));
      return {
        ...meta,
