...
class BadgesService {
  ...
  }: {channelId: EntityId, parentId?: EntityId, userId: EntityId, lastRead: Date, lastMessageId: EntityId, count?: number}) {
  ...
  async messageSent({channelId, parentId, messageId, userId}: {channelId: EntityId, parentId?: EntityId, messageId: EntityId, userId: EntityId}) {
    ...
    other.filter((badge: Badge) => badge.userId !== userId).forEach((badge: Badge) => {
