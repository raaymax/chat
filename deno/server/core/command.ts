...
export type Definition<T extends string, U extends v.BaseSchema<any, any,any>> = {
  type: T,
  body: U,
}

export type Handler<U extends v.BaseSchema<any, any,any>> = (body: v.InferOutput<U>, core: Core) => Promise<void | EntityId | null>;

export type Command<T extends string, U extends v.BaseSchema<any, any,any>> = {
    type: T,
    def: Definition<T, U>,
    handler: (evt: v.InferInput<U>, core: Core) => Promise<void | EntityId | null>,
}

export function createCommand<T extends string, U extends v.BaseSchema<any, any,any>>(def: Definition<T, U>, fn: Handler<U>): Command<T, U>{
  return {
    type: def.type,
    def,
...
export function buildCommandCollection<T extends {type: string}[]>(events: T): CommandDirectory<T> {
...
export type EventFrom<T> = T extends Command<infer T, infer U> ? {type: T, body: v.InferInput<U>} : never;
