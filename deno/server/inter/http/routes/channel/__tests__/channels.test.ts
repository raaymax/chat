...
Deno.test("POST /channels - unauthorized", async () => {
  ...
  try{
    await agent.request().post('/channels').emptyBody().expect(401);
    await agent.request().get('/channels').expect(401);
    await agent.request().get('/channels/xyz').expect(401);
    await agent.request().delete('/channels/xyz').emptyBody().expect(401);
  } finally {
    await agent.close();
  }
})
...
Deno.test("/channels - server sent events", async () => {
  const agent = await Agent.from(app);
  const { token } = await login(agent);
  const { token: memberToken, userId } = await login(agent, 'member');
  await repo.channel.removeMany({name: 'test', channelType: ChannelType.PUBLIC});
  const events = agent.events('/sse', { headers: { Authorization: `Bearer ${memberToken}` } });
  try{
    {
