...
Deno.test("/api/channels/:channelId/read-receipts", async () => {
  return await Agent.test(app, {type: 'handler'}, async (agent) => {
    return await Chat.init(repo, agent)
      ...
      .sendMessage({flat: "Hello", message: {text: 'Hello'}, clientId: "test0"}, (msg, {state}) => {
        state.messageId = msg.id;
      })
      .getChannelReadReceipts((receipts: any, self) => {
        const receipt = receipts.find((r: any) => r.channelId === self.channelId);
...
Deno.test("/api/channels/:channelId/read-receipts - SSE", async () => {
  return await Agent.test(app, {type: 'handler'}, async (agent) => {
    ...
    await admin.createChannel({name: "test-read-receipts2", users: [member.userId]});
    await member.gotoChannel("test-read-receipts2");
    await admin.sendMessage({flat: "Hello", message: {text: 'Hello'}, clientId: "test1"});
...
Deno.test("/api/read-receipts", async () => {
  return await Agent.test(app, {type: 'handler'}, async (agent) => {
    ...
    await admin.createChannel({name: "test-read-receipts3", users: [member.userId]});
    await member.gotoChannel("test-read-receipts3");
    await admin.sendMessage({flat: "Hello", message: {text: 'Hello'}, clientId: "test2"});
    await member
      .getMessages({}, async (messages: any, {state}) => {
        state.messageId = messages[0].id;
      })
      .updateReadReceipts(({state}) => state.messageId)
    await admin.sendMessage({flat: "Hello", message: {text: 'Hello'}, clientId: "test3"});
    await admin.sendMessage({flat: "Hello", message: {text: 'Hello'}, clientId: "test4"});
    await member.getReadReceipts((receipts: any) => {
      assertEquals(receipts.length, 1);
      const receipt = receipts.find((r: any) => r.channelId === member.channelId);
